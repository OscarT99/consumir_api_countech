...
const url = 'https://api-countech.onrender.com/api/pedido'
...
const listarDatos= async()=>{
    ...
    .then(function(data){
        ...
        return listaPedidos.map(function(pedido){
          const fechaOrdenTrabajo = new Date(pedido.fechaOrdenTrabajo);
          const formattedFechaOrdenTrabajo = fechaOrdenTrabajo.toISOString().split('T')[0] 
          const fechaEntregaOrdenTrabajo = new Date(pedido.fechaEntregaOrdenTrabajo);
          const formattedfechaEntregaOrdenTrabajo = fechaEntregaOrdenTrabajo.toISOString().split('T')[0]
            respuesta+=`<tr><td>${pedido.cliente}</td>`+
            `<td>${pedido.contacto}</td>`+
            `<td>${pedido.ordenTrabajo}</td>`+
            `<td>${formattedFechaOrdenTrabajo}</td>`+
            `<td>${formattedfechaEntregaOrdenTrabajo}</td>`+
            `<td>${pedido.formaPago}</td>`+
            `<td>${pedido.valorTotal}</td>`+
            `<td>${pedido.observaciones}</td>`+            
            `<td>${pedido.estado}</td>`+
            `<td><button type="button"  data-bs-toggle="modal"  data-bs-target="#exampleModal" onclick='editar(${JSON.stringify(pedido)})'><i class="fa-solid fa-file-pen fa-lg" style="color: #000000;"></i></button></td>
            <td><a href='#'  onclick='eliminar(${JSON.stringify(pedido)})' type="button"><i class="fa-solid fa-trash fa-xl" style="color: #940000;"></i></a></td></tr>`
...
const editar = (pedido) => {
    ...
    const fechaOrdenTrabajo = new Date(pedido.fechaOrdenTrabajo);
    const formattedFechaOrdenTrabajo = fechaOrdenTrabajo.toISOString().split('T')[0];
    document.getElementById('fechaOrdenTrabajo').value = formattedFechaOrdenTrabajo;

    const fechaEntregaOrdenTrabajo = new Date(pedido.fechaEntregaOrdenTrabajo);
    const formattedFechaEntregaOrdenTrabajo = fechaEntregaOrdenTrabajo.toISOString().split('T')[0];
    document.getElementById('fechaEntregaOrdenTrabajo').value = formattedFechaEntregaOrdenTrabajo;
