...
const url = 'https://api-countech.onrender.com/api/cliente'
...
const listarDatos= async()=>{
    ...
    .then(function(data){
        ...
        return listaClientes.map(function(cliente){
            respuesta+=`<tr><td>${cliente.tipoCliente}</td>`+
            ...
            `<td><button type="button"   data-bs-toggle="modal"  data-bs-target="#exampleModal" onclick='editar(${JSON.stringify(cliente)})'><i class="fa-solid fa-user-pen fa-lg" style="color: #000000;"></i></button></td>
            <td><a href='#'    onclick='eliminar(${JSON.stringify(cliente)})' type="button"><i class="fa-solid fa-trash fa-xl" style="color: #940000;"></i></a></td></tr>`
...
const registrar = async() =>{
    ...
    if (validarNumeroIdentificacionRespuesta && validarRazonSocialRespuesta && validarNombreComercialRespuesta && validarCiudadRespuesta && validarDireccionRespuesta && validarTelefonoRespuesta && validarCorreoRespuesta && validarContactoRespuesta){
        let _tipoCliente = document.getElementById('tipoCliente1').value
        ...
        .then(json => {
            ...
            setTimeout(() =>{
                window.location.href = 'listarCliente.html';
            },1000);  
        })
    }
}
